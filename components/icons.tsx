...
export function FlowerLogo({ className = "h-8 w-8" }: { className?: string }) {
  return (
    <svg viewBox="0 0 40 40" className={className} aria-hidden="true">
      <g transform="translate(20 20)">
        {[0, 72, 144, 216, 288].map((a) => (
          <ellipse key={a} cx="0" cy="-9" rx="6.5" ry="9" fill="#e0567f" transform={`rotate(${a})`} />
        ))}
        {[36, 108, 180, 252, 324].map((a) => (
          <ellipse key={a} cx="0" cy="-6" rx="4.2" ry="6" fill="#f6a3bd" transform={`rotate(${a})`} />
        ))}
        <circle r="4.2" fill="#f29e1f" />
        <circle r="1.8" fill="#c96f00" />
      </g>
    </svg>
  );
}
...
export function ArrowIcon({ className = "h-4 w-4 rtl:rotate-180" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
...
export function ZoomIcon({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true">
      <circle cx="11" cy="11" r="6.5" />
